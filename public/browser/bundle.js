          ...
          document.getElementById("startButtonStart").onclick = () => {
            if (!this.currentAlgorithm) {
              document.getElementById("startButtonStart").innerHTML =
                '<button class="btn btn-default navbar-btn" type="button">Pick an algorithm from the list and press visualize!</button>';
            } else {
        ...
        Board.prototype.changeStartNodeImages = function () {
          ...
          if (unweighted.includes(this.currentAlgorithm)) {
            if (this.currentAlgorithm === "dfs") {
              document.getElementById(
                "algorithmDescriptor"
              ).innerHTML = `${name} <i><b>does not guarantee</b></i> the shortest path!`;
            } else {
              document.getElementById(
                "algorithmDescriptor"
              ).innerHTML = `${name} <i><b>guarantees</b></i> the shortest path!`;
            }
        ...
        Board.prototype.toggleTutorialButtons = function () {
          ...
          function nextPreviousClick() {
            if (counter === 1) {
              document.getElementById(
                "tutorial"
              ).innerHTML = `<h3>Welcome to Derek's Graph Pathfinding Visualizer!</h3>
      <br />
      <h6>
        This is a short tutorial on how this application works and what you can
        do with it!
      </h6>
      <br />
      <p>
        If you would like to skip this, please press the "Skip Tutorial" button.
        Otherwise, please continue on to learn all about this project and how it
        came to be!
      </p><div id="tutorialCounter">1/3</div><button id="nextButton" class="btn btn-default navbar-btn" type="button">Next</button><button id="previousButton" class="btn btn-default navbar-btn" type="button">Previous</button><button id="skipButton" class="btn btn-default navbar-btn" type="button">Skip Tutorial</button>`;
            } else if (counter === 2) {
              document.getElementById(
                "tutorial"
              ).innerHTML = `<h3>Functionality of Visualizer</h3><h6>There is a start node and end node on the screen. The Visualizer will find the path from the start to the end node, depending on which specific algorithm chosen. You can move the start and the end node both before and after the visualization to see the effects.</h6><p>By clicking on the grid, you can create "walls" which create a node in which the visualizer cannot pass thorugh. There are also mazes that can be generated through the dropdown on the navbar. Bombs are a type of special node that when placed, the visualizer will find the shortest path to the bomb and then the end node.</p><div id="tutorialCounter">${counter}/3</div><button id="nextButton" class="btn btn-default navbar-btn" type="button">Next</button><button id="previousButton" class="btn btn-default navbar-btn" type="button">Previous</button><button id="skipButton" class="btn btn-default navbar-btn" type="button">Skip Tutorial</button>`;
            } else if (counter === 3) {
              document.getElementById(
                "tutorial"
              ).innerHTML = `<h3>Thank You For Trying This Application!</h3><h6>I hope you are excited to try out this application!</h6><p>I hope you have as much fun visualizing these popular pathfinding algorithms as I did making this application. I learned a great deal about how these magnificent algorithms work and am excited for what's next in my software development journey.</p><p>If you would like to check out other cool projects that I have made like this one, please visit my GitHub: <a href="https://github.com/dxu5" target="_blank">My GitHub</a></p><div id="tutorialCounter">${counter}/3</div><button id="finishButton" class="btn btn-default navbar-btn" type="button">Finish</button>`;
